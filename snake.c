...
bool checkCollisionWithEdge(Player *player);
...
int main(void){
    ...
    while (!WindowShouldClose()){
        ...
        case GAME:
            ...
            if(checkCollisionWithSnake(&game.player) || checkCollisionWithEdge(&game.player)){
                currentScreen = OVER;
            }
...
bool checkCollisionWithEdge(Player *player){
    if(player->cellsPos[0].x >= cellSize*cellNum || player->cellsPos[0].x < 0 || player->cellsPos[0].y >= cellSize*cellNum || player->cellsPos[0].y < 0){
        return true;
    }
    return false;
}
